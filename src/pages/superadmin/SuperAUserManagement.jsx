...
import {
  ...
  IconButton, Stack, InputAdornment, Avatar, Tooltip, Switch,
  FormControlLabel, LinearProgress, Tabs, Tab, Pagination, Alert, Snackbar,
...
const UserManagement = () => {
  ...
  const cargarUsuarios = async () => {
    try {
      setLoading(true);
      // Traemos los usuarios agrupados por instancia
      const dataGrouped = await usuarioService.findAgrupadosPorInstancia();
      // Transformamos a array plano para tabla
      const data = Object.entries(dataGrouped).flatMap(([instanciaId, usuarios]) => 
        usuarios.map(u => ({ 
          ...u, 
          instanciaId: parseInt(instanciaId), 
          instanciaNombre: u.instanciaNombre || `Instancia ${instanciaId}` 
        }))
      );
      setUsers(data);
      // Generamos lista de instancias únicas
      const instancias = [...new Set(data.map(u => u.instanciaNombre))].filter(Boolean);
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(u => {
      const matchesSearch = !searchTerm ||
        ...
        (u.instanciaNombre?.toLowerCase() || '').includes(searchTerm.toLowerCase());
      ...
      const matchesInstancia = selectedInstancia === 'all' ? true : u.instanciaNombre === selectedInstancia;
  ...
  const handleCreateUser = async (userData, passwordValue) => {
    ...
    try {
      ...
      mostrarSnackbar(error.error || 'Error al crear el usuario', 'error');
    }
  };
        ...
        <Paper elevation={0} sx={{ p: 2, bgcolor: '#f8f9fa' }}>
          <Stack direction="row" spacing={2} sx={{ mb: 2 }}>
            ...
            <FormControl size="small" sx={{ flex: 1 }}>
              <InputLabel id="instancia-filter-label">Filtrar por instancia</InputLabel>
              <Select
              ...
              >
                ...
                {availableInstancias.map((instancia) => (
                  <MenuItem key={instancia} value={instancia}>{instancia}</MenuItem>
                ))}
              </Select>
            </FormControl>
          </Stack>
          ...
          <Table stickyHeader size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell sx={{ fontWeight: 'bold', color: colors.primary.dark, width: '15%' }}>Instancia</TableCell>
                <TableCell sx={{ fontWeight: 'bold', color: colors.primary.dark, width: '15%' }}>Último Acceso</TableCell>
                <TableCell sx={{ fontWeight: 'bold', color: colors.primary.dark, width: '10%' }}>Estado</TableCell>
                  ...
                  >
                    ...
                    <TableCell>
                      <Chip 
                        ...
                        size="small" 
                        sx={{
                          bgcolor: u.estado === 'ACTIVO' ? colors.secondary.main : colors.primary.light,
                          color: 'white', 
                          fontSize: '0.75rem'
                        }} 
                      />
