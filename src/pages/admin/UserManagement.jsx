...
const UserManagement = () => {
  ...
  const handleSaveRole = async (userData) => {
    const esComite = esRolComite(userData.rolNombre);
    if (esComite && !userData.rolEspecifico) {
      mostrarSnackbar('Debe seleccionar un cargo específico para el rol de Comité', 'error');
      return;
    ...
    try {
      ...
      const updateData = {
        ...
        rolNombre: userData.rolNombre,
        rolEspecifico: esComite ? userData.rolEspecifico : null,
        regionNombre: userData.regionNombre || usuarioActual.regionNombre,
      ...
      mostrarSnackbar(
        `Rol de ${userData.nombre} actualizado a ${userData.rolNombre}${esComite && userData.rolEspecifico ? ` - ${userData.rolEspecifico}` : ''}`,
        'success'
      );
      handleCloseModals();
    } catch (error) {
