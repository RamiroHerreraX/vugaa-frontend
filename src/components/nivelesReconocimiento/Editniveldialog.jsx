...
const EditNivelDialog = ({ open, onClose, onSave, nivel, saving }) => {
    const { user } = useAuth();
    ...
    // Cargar programas cuando se abre el diálogo
    useEffect(() => {
    ...
    // Cargar datos del nivel cuando cambia
    useEffect(() => {
        if (nivel && open) {
            let programasSeleccionados = [];
            
            // Intentar parsear las reglasJson si existen
            if (nivel.reglasJson) {
                try {
                    const reglas = JSON.parse(nivel.reglasJson);
                    programasSeleccionados = reglas.programas || [];
                } catch (e) {
            ...
            // Limpiar errores al cargar nuevo nivel
            setErrors({});
        }
    }, [nivel, open]);
    ...
    const handleSave = () => {
        if (!validate() || !nivel) return;

        // Obtener versión actual o iniciar en 1
        ...
        if (nivel.reglasJson) {
            try {
                const reglas = JSON.parse(nivel.reglasJson);
                currentVersion = reglas.version || 1;
                fechaCreacion = reglas.fechaCreacion || new Date().toISOString();
            } catch (e) {
                console.error('Error al parsear reglasJson:', e);
            }
        }
        ...
        <Dialog open={open} onClose={handleClose} maxWidth="md" fullWidth
            ...
            <DialogTitle sx={{ borderBottom: `1px solid ${colors.primary.light}` }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    <Typography variant="h6" sx={{ color: colors.primary.dark }}>
                        Editar Nivel de Reconocimiento
                    </Typography>
                </Box>
            </DialogTitle>
            ...
            <DialogActions sx={{ p: 2, borderTop: `1px solid ${colors.primary.light}` }}>
                ...
                <Button 
                    ...
                    {saving ? 'Guardando...' : 'Actualizar Nivel'}
                </Button>
            </DialogActions>
        </Dialog>
    );
